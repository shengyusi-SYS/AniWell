...
class hlsRequestHandler {
    ...
    handler = (app) => {
        try {
                ...
                app.use('/api/localFile/output', debounce(async (req, res, next) => {
                        ...
                        if (tryTimes >= 20) {
                            ...
                            return stat(path.resolve(settings.tempPath, 'output', targetSegment + '.ts')).then((result) => {
                                if (_this.videoIndex[targetSegment].state == 'done') {
                                    logger.debug('hlsRequestHandler /api/localFile/output', 'send', targetSegment);
                                    tryTimes = 0;
                    ...
                    if (req.path == '/index.m3u8') {
                        ...
                        if (targetSegmentId < Number(_this.lastTargetId)) {
                            if (_this.videoIndex[targetSegment].state != 'done') {
                                logger.info('hlsRequestHandler handler 3', 'back----------', targetSegmentId, _this.lastTargetId);
                                ...
                                if (_this.HlsProcessController.currentProcess.id <= targetSegmentId) {
                                    logger.debug('hlsRequestHandler handler 3', 'continue----------', targetSegment);
                                } else {
                                    logger.debug('hlsRequestHandler handler 3', 'backcheck----------', targetSegment);
                            ...
                            if (_this.videoIndex[targetSegment].state != 'done') {
                                ...
                                if (_this.HlsProcessController.currentProcess.id <= targetSegmentId) {
                                    ...
                                    if (nextProcessId > endId) {
                                        ...
                                        logger.debug('hlsRequestHandler handler 3', 'jump continue', nextProcessId);
                                        await _this.HlsProcessController.killCurrentProcess()
                                        await _this.HlsProcessController.generateHlsProcess(`index${nextProcessId}`);
                                    }
                                }
            ...
            //hls缓存清理
            used = app._router.stack.findIndex(v => v.regexp.toString().includes('clearVideoTemp'))
...
module.exports = hlsRequestHandler
