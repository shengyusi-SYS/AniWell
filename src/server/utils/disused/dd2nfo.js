...
function dd2nfo(dandanplayPath, fullUpdate = false, overwrite = false) {
    ...
    try {
        var library = JSON.parse(
            fs.readFileSync(path.resolve(dandanplayPath, "library.json")),
        ).VideoFiles
    } catch (error) {
        return Promise.reject("未检测到弹弹Play")
    }
    ...
    for (const ap in update) {
        ...
        if (!overwrite) {
            ...
            let oldNfo, newNfo
            try {
                oldNfo = fs.readFileSync(
                    path.resolve(path.dirname(anime.Path), `${path.parse(anime.Name).name}.nfo`),
                )
    ...
    for (const aniPath in season) {
        ...
        try {
            ...
            try {
                ...
                let task = got({
                    url: `https://api.dandanplay.net/api/v2/search/anime?keyword=${encodeURIComponent(
                        season[aniPath].tvshow.title,
                    )}`,
                    method: "get",
                })
                    .then((result) => {
    ...
    return Promise.all(searchQueue)
        .catch((err) => {
            console.log(err)
        })
        .then((res) => {
