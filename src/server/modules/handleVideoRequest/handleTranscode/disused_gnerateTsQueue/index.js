const { logger, transcodeLogger } = require("@s/utils/logger")
const { settings, ffmpegSuffix } = require("@s/utils/init")
const { event, stat } = require("@s/utils")
    ...
    try {
        ...
        for (const segment in videoIndex) {
            ...
            let process = async () => {
                ...
                transState = "doing"
                if (
                    Number(segment.replace("index", "")) == Object.keys(videoIndex).length - 1 &&
                ...
                ffmpeg.stderr.on("data", async function (stderrLine) {
                    ...
                    if (/Opening.*for writing/.test(stderrLine) && !/m3u8/i.test(stderrLine)) {
                        let writingSegment = path.parse(
                            path.parse(/'.*'/.exec(stderrLine)[0]).name,
                        ).name
                            ...
                            for (; lastWriteId <= writingSegmentId - 1; lastWriteId++) {
                                ...
                                stat(
                                    ...
                                    .then((result) => {
                                        ...
                                        logger.debug(
                                            "debug",
                                            "reloaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaad",
                                            result,
                                            tempLostSegment,
                                            FFmpegProcess[tempLostSegment],
                                        )
                                    })
                                    .catch((err) => {
                                        logger.error(
                        ...
                        if (writingSegmentId != ffmpeg.id) {
                            let completedSegment = `index${
                                writingSegmentId - 1 >= 0 ? writingSegmentId - 1 : 0
                            }`
