...
const sizeChange = watch(pageSize, (newSize, oldSize) => {
    ...
    if (oldSize != undefined && libraryData.value.libName !== "search") {
...
let touchTimer
async function touchCard(e: TouchEvent, cardData: libraryData) {
    if (e.touches.length === 1) {
        touchTimer = setTimeout(() => {
            ...
            floatMenuCoordinate.value.x = clientX
            floatMenuCoordinate.value.y = clientY
            floatMenuOpen.value = true
        }, 500)
    }
}
...
//拖动排序
const dragSortList = ref()
const sortable = ref<Sortable | null>(null)
console.log(dragSortList.value)
watch(dragSortList, (newEl) => {
    if (newEl) {
        sortable.value = Sortable.create(dragSortList.value, {
            animation: 150,
            onEnd(e) {
                const { newIndex, oldIndex } = e
                console.log(e, newIndex, oldIndex)

                if (newIndex && oldIndex && newIndex !== oldIndex) {
                    const moving = sortList.value.splice(oldIndex - 1, 1)[0]
                    sortList.value.splice(newIndex - 1, 0, moving)
                    nextTick(() => {
                        router.replace({
                            name: "library",
...
<template>
    <div ref="library" class="library-base col">
        ...
        <div
            v-if="libraryData.libName === 'overview' || libraryData.libName === 'search'"
            class="library-overview col"
        >
                    ...
                    <div class="library-info-bar">
                        ...
                        >
                            ...
                            <div v-show="sortMenuOpen" class="info-sort-menu">
                                <div class="col info-sort-list" ref="dragSortList">
                                    <div>拖动排序</div>
                                    <div
            ...
            >
                ...
                >
                    <Card
                        ...
                        @click.right.prevent="
                            async (e) => {
                                await clickCard(e, cardData)
                            }
                        "
                        @touchstart="
                            async (e) => {
                                await touchCard(e, cardData)
                            }
                        "
                        @touchend="touchEnd"
                        @touchmove="touchEnd"
