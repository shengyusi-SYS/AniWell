...
import { settings, settingsList, proxySettings, libraryIndex, mergeSettings } from './utils/init';
...
var SID: string
var cookieTimer: NodeJS.Timeout
var checkCookie: boolean = true
var maindataCache: { torrents: object } = { torrents: {} }
var videoHandler
var bannedSIDs: string[] = []
...
app.use("/api/localFile/library", (req, res, next) => {
    try {
        var library = JSON.parse(fs.readFileSync(path.resolve('./libraryIndex.json')).toString())
        res.send(library)
    } catch (error) {
